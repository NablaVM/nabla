...
TEST(PushPop, Push)
{
    for(int16_t j = 0; j < 50; j++)
    {
        ...
        expectedIns.bytes[0] = NABLA::VSYS::INS_PUSH;
        ...
        NABLA::Bytegen::Instruction ins = byteGen.createPushInstruction(
            stackType,
            expectedIns.bytes[2]
        );
...
TEST(PushPop, Pop)
{
    for(int16_t j = 0; j < 50; j++)
    {
        ...
        expectedIns.bytes[0] = NABLA::VSYS::INS_POP;
        ...
        NABLA::Bytegen::Instruction ins = byteGen.createPopInstruction(
            stackType,
            expectedIns.bytes[1]
        );
...
TEST(PushPop, Pushw)
{
    for(int16_t j = 0; j < 50; j++)
    {
        ...
        expectedIns.bytes[0] = NABLA::VSYS::INS_PUSHW;
        expectedIns.bytes[1] = getStackAddress(stackType);
        expectedIns.bytes[2] = integerToRegister(getRandom8(0, 9)) ;
        expectedIns.bytes[3] = 0xFF;
        expectedIns.bytes[4] = 0xFF;
        ...
        NABLA::Bytegen::Instruction ins = byteGen.createPushwInstruction(
            stackType,
            expectedIns.bytes[2]
        );
...
TEST(PushPop, Popw)
{
    for(int16_t j = 0; j < 50; j++)
    {
        ...
        expectedIns.bytes[0] = NABLA::VSYS::INS_POPW;
        expectedIns.bytes[1] = integerToRegister(getRandom8(0, 9)) ;
        expectedIns.bytes[2] = getStackAddress(stackType);
        expectedIns.bytes[3] = 0xFF;
        expectedIns.bytes[4] = 0xFF;
        ...
        NABLA::Bytegen::Instruction ins = byteGen.createPopwInstruction(
            stackType,
            expectedIns.bytes[1]
        );
