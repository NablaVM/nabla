...
TEST(ReturnTests, Return)
{
    for(int16_t j = 0; j < 50; j++)
    {
        SOLACE::Bytegen::Instruction expectedIns;

        expectedIns.bytes[0] = SOLACE::MANIFEST::INS_RET;
        ...
        SOLACE::Bytegen::Instruction ins = byteGen.createReturnInstruction();
